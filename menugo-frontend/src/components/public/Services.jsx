...
const Services = () => {
  ...
  const stats = [
    { value: '500+', label: 'Restaurants live' },
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
          ...
          >
            <motion.div variants={fadeInUp} className="max-w-2xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3 sm:gap-4">
                <Link
                  to="/contact"
                  className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-orange-600/30 transition-all hover:-translate-y-0.5 hover:bg-orange-700"
                >
                  Book a Demo
                ...
                <a
                  href="#pricing"
                  className="inline-flex items-center rounded-xl border border-slate-200 bg-white/80 px-6 py-3 text-sm font-semibold text-slate-700 transition-all hover:-translate-y-0.5 hover:border-orange-300 hover:text-orange-600"
                >
                  View Pricing
                </a>
                <Link
              ...
              <div className="relative rounded-2xl border border-white/70 bg-white/90 p-3 shadow-2xl backdrop-blur-md">
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
                  {stats.map((item, index) => (
                    <motion.div
                      key={item.label}
                      className="rounded-xl border border-slate-100 bg-white/70 p-3 text-center shadow-sm backdrop-blur-sm"
                      initial={{ opacity: 0, y: 10 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }}
                      transition={{ delay: 0.2 + index * 0.08 }}
                    >
                      <p className="text-lg font-black text-orange-600 sm:text-xl">{item.value}</p>
                      <p className="mt-1 text-[11px] font-medium text-slate-500 sm:text-xs">{item.label}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {services.map((service, index) => (
              ...
              >
                ...
                <Link
                  to="/contact"
                  className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-orange-600 transition-colors hover:text-orange-800"
                >
                  Learn more
      ...
      <section className="bg-slate-50 py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-10 max-w-3xl text-center">
            <h2 className="text-3xl font-black tracking-tight text-slate-900 md:text-4xl">Advanced capabilities</h2>
            <p className="mt-3 text-lg text-slate-600">Powerful features to elevate your guest experience.</p>
          </div>
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-10 max-w-3xl text-center">
            <h2 className="text-3xl font-black tracking-tight text-slate-900 md:text-4xl">Works with your stack</h2>
            <p className="mt-3 text-lg text-slate-600">Seamless integrations with tools you already use.</p>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-8 md:gap-12">
            {integrations.map((item, idx) => (
              <motion.div
                ...
                className="flex flex-col items-center gap-2 rounded-xl bg-slate-50 px-6 py-3 shadow-sm"
              >
      ...
      <section className="bg-orange-50 py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-12 max-w-3xl text-center">
            ...
            <p className="mt-3 text-lg text-slate-700">Real results from real teams.</p>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {pricing.map((plan, index) => (
              ...
              >
                {plan.recommended && (
                  <div className="bg-orange-600 px-4 py-2 text-center text-sm font-semibold text-white">Recommended</div>
                )}
                <div className="p-6">
                  <h3 className="text-2xl font-bold text-slate-900">{plan.name}</h3>
                  <div className="mt-4 flex items-end gap-1">
                    <span className="text-4xl font-black text-slate-900">{plan.price}</span>
                    {plan.period !== 'Custom' && <span className="pb-1 text-slate-500">/{plan.period}</span>}
                  </div>
                  ...
                  <Link
                    to="/contact"
                    className={`mt-8 inline-flex w-full items-center justify-center rounded-xl px-6 py-3 text-sm font-semibold transition-colors ${
                      plan.recommended
      ...
      <section className="relative overflow-hidden bg-slate-900 py-16 sm:py-20">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <motion.div variants={popIn} className="mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-6 py-3 text-sm font-bold text-slate-900 transition-all hover:-translate-y-0.5 hover:bg-orange-400"
            >
              Start Free Trial
              <ArrowRightIcon className="h-5 w-5" />
            </Link>
            <Link
              to="/login"
              className="inline-flex items-center rounded-xl border border-slate-600 px-6 py-3 text-sm font-semibold text-slate-100 transition-all hover:-translate-y-0.5 hover:border-slate-400 hover:text-white"
            >
              Sign In
            </Link>
          </motion.div>
