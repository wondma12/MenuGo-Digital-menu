      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-900">{currentPlan?.billingCycle}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Price</p>
            <p className="font-semibold text-gray-900">${currentPlan?.price}/month</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Next Billing</p>
            <p className="font-semibold text-gray-900">{new Date(currentPlan?.nextBillingDate).toLocaleDateString()}</p>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {availablePlans?.map((plan) => (
            <div key={plan.id} className={`bg-white rounded-xl border-2 p-6 ${currentPlan?.tier === plan.tier ? 'border-primary-500' : 'border-gray-200'}`}>
